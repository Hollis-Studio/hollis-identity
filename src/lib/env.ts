 ...
 * - Core: DATABASE_URL, JWT_SECRET, JWT_ISSUER, JWT_AUDIENCES, BCRYPT_PEPPER, PORT, LOG_LEVEL
...
const secretSchema = z.string().refine(
  (val) => {
    ...
    if (FORBIDDEN_SECRET_VALUES.some((forbidden) => lowerVal.includes(forbidden))) return false;
...
const optionalSecretSchema = z.string().optional().refine(
  (val) => {
    if (!val) return true;
    const isProduction = process.env.NODE_ENV === "production";
...
const urlSchema = z.string().url().refine(
  ...
  { message: "Must be a valid URL with http://, https://, or postgresql:// protocol" },
);
...
const envSchema = z.object({
  ...
  BCRYPT_PEPPER: z.string().optional(),
  ...
  PASSWORD_PEPPER: z.string().min(32, "Password pepper must be at least 32 characters").optional(),
  ACCESS_TOKEN_DENYLIST_ENABLED: z.string().transform((val) => val !== "false").default(true),
...
function pickDataMigrationModeEnv(rawEnv: RawDataMigrationModeEnv = process.env): RawDataMigrationModeEnv {
  return {
...
export function isDataMigrationModeEnabled(rawEnv: RawDataMigrationModeEnv = process.env): boolean {
  const result = dataMigrationModeEnvSchema.safeParse(pickDataMigrationModeEnv(rawEnv));
  if (!result.success) return false;
  return result.data.DATA_MIGRATION_RUN === "1" || result.data.RUN_DATA_MIGRATIONS === "1";
}

export function hasDataMigrationModeFlag(rawEnv: RawDataMigrationModeEnv = process.env): boolean {
  const flags = pickDataMigrationModeEnv(rawEnv);
  return flags.DATA_MIGRATION_RUN !== undefined || flags.RUN_DATA_MIGRATIONS !== undefined;
}
...
export function validateEnvOnStartup(): void {
  ...
  if (isProduction) {
    if (!validated.APPLE_SERVICE_ID) {
      warnings.push("APPLE_SERVICE_ID not set. Apple Sign In will be unavailable.");
    }
    if (!validated.GOOGLE_CLIENT_ID) {
      warnings.push("GOOGLE_CLIENT_ID not set. Google Sign In will be unavailable.");
    }
